'''
    THIS SCRIPT IS AN EXAMPLE OF HOW TO USE THE ROC_AUC_EXPERIMENT
...
for lamb in lambs:
    EXP_ID=str(EXP_INT)
    sim_names = {'cos_sim', 'jaccard_best_avg', 'resnik_best_avg',
                 'lin_best_avg', 'jc_best_avg'}
    fig, ax = plt.subplots(2, 2)
    i = 0
    for noise_ptg in noise_ptgs:
        aucs = {'cos_sim': [], 'jaccard_best_avg': [], 'resnik_best_avg': [],
                'lin_best_avg': [], 'jc_best_avg': []}
        tprs = {'cos_sim': [], 'jaccard_best_avg': [], 'resnik_best_avg': [],
                'lin_best_avg': [], 'jc_best_avg': []}
        fprs = {'cos_sim': [], 'jaccard_best_avg': [], 'resnik_best_avg': [],
                'lin_best_avg': [], 'jc_best_avg': []}
        interp_tpr = {}
        ...
        for exp in range(number_experiments):
            ...
            experiment = ROC_AUC_EXPERIMENT(source=source, EXP_ID=EXP_ID,
                                            exp_id=exp_id)
        ...
        for sim in sim_names:
            ...
            axis.plot(mean_fpr, mean_tpr[sim],
                    label='{} $\pm$ 1 SD (AUC = {} $\pm$ {})'.format(
                        sim, round(mean_auc[sim], 3), round(std_auc[sim], 3)
                    ),
                    lw=2, alpha=.8)
            axis.fill_between(mean_fpr, tprs_lower[sim],
                              tprs_upper[sim], alpha=.2)
            color = next(axis._get_lines.prop_cycler)['color']

        axis.plot([0, 1], [0, 1], linestyle='--', lw=2)
        axis.set(xlim=[-0.05, 1.05], ylim=[-0.05, 1.05],
               title="Lambda={} ; {}% as many noise terms added".format(
                   lamb, noise_ptg*100)
                 )
