...
def compare_patients(hpoic, patient1, patient2, patient_embeddings, scores=None, use_aoo=False):
...
def script(patient_filename, hpo_filename, #output_table,
           disease_phenotype_filename, patient2vec_file, pkl_with_sims,
    ...
    patient_embeddings = load_object(patient2vec_file)
    # of = open(output_table, 'w')
    ...
    for i in range(len(patients)):
        ...
        for o in compare_against:
            ...
            sims = compare_patients(hpoic, patient, o, patient_embeddings, scores=scores, use_aoo=use_aoo)

            if header is None:
                header = sorted(sims)

                for p in patients:
                    patient_similarities[get_id(p)] = {}
                    for sim in header:
                        patient_similarities[get_id(p)][sim] = {}

                # of.write('{}\n'.format('\t'.join(['A', 'B'] + header)))
                print('\t'.join(['A', 'B'] + header))

            for sim in header:
                patient_similarities[id1][sim][id2] = sims[sim]
                # patient_similarities[id2][sim][id1] = sims[sim]

            sim_strs = ['{:.6f}'.format(sims[sim]) for sim in header]

            for sim, sim_str in zip(header, sim_strs):
                logging.debug('{}: {}'.format(sim, sim_str))
            # of.write('{}\n'.format('\t'.join([id1, id2] + sim_strs)))
            print('\t'.join([id1, id2] + sim_strs))

    save_object(patient_similarities, pkl_with_sims)
    # of.close()
...
def parse_args(args):
    ...
    # parser.add_argument('output_table', default='../_data/patients/decipher_patient_sims.tab')
    parser.add_argument('patient2vec_file', default='../_data/patients/decipher_patient_embeddings.pkl')
    parser.add_argument('pkl_with_sims', default='../_data/patients/decipher_patient_sims.pkl')
    ...
    parser.add_argument('-s', '--score', dest='scores', action='append', default=[],
                        choices=['jaccard', 'resnik', 'lin', 'jc', 'owlsim', 'jz', 'ui', 'simgic', 'nsimgic', 'icca', 'cos_sim'],
