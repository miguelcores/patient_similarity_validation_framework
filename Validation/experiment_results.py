...
class ROC_AUC_EXPERIMENT():
    ...
    def plot_it(self):
        ...
        for sim in self.sim_names:
            plt.plot(self.fpr[sim], self.tpr[sim],
                     lw=lw,
                     label='ROC curve for {} (area = {})'.format(
                         sim, round(self.roc_auc[sim], 3))
                     )
    ...
    def load_results(self, sim_name, sim_array):
        if self.m != (len(sim_array)+1): return
            ...
            self.fpr[sim_name], self.tpr[sim_name], self.thresholds[sim_name] = \
                roc_curve(self.y, sim_array)
            ...
            for sim in patient_similarities[patient]:
                if sim == 'cos_sim':
                    for patient_compared in patient_similarities[patient][sim]:
                        self.cos_sim.append(
                            patient_similarities[patient][sim][patient_compared]
                        )
                        if patients_disease[patient_compared] == \
                                patients_disease[patient]:
                            self.y.append(1)
                        else:
                            self.y.append(0)
                ...
                if sim == 'jaccard_best_avg':
                    self.jaccard_best_avg = \
                        self.append_similarity_score(
                            self.jaccard_best_avg,
                            patient_similarities[patient][sim]
                        )
